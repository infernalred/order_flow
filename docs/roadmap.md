...
- [ ] Описать типизированную конфигурацию приложения.
- [ ] Загружать настройки из environment variables.
- [ ] Добавить `.env.example` без секретов.
- [ ] Валидировать обязательные настройки при старте.
- [ ] Настроить структурированные логи через `slog`.
- [ ] Добавить request ID и correlation ID в HTTP-логи.
